...
class CMPC { public:
	...
	CMPC(NetIOMP<nP> * io[2], ThreadPool * pool, int party, BristolFormat * cf, int * num_and_gates = nullptr, bool * _delta = nullptr, int ssp = 40) {
		this->party = party;
		this->io = io[0];
		this->cf = cf;
		this->ssp = ssp;
...
//		ret.get();
	...
	void online (uint8_t *input, uint8_t *output) 
	{
		...
		else 
		{
			...
			{
				/* cout << "Gate: " << i << endl;
                                cout << "Type: " << cf->gates[i+3] << endl;
                                cout << "Input 1: " << cf->gates[i] << endl;
                                cout << "Input 2: " << cf->gates[i+1] << endl;
                                cout << "Output: " << cf->gates[i+2] << endl; */
				switch(cf->gates[i+3])
				{
					case XOR_GATE:
