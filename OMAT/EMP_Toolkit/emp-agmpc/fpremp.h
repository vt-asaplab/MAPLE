...
class FpreMP { public:
	...
	void compute(block * MAC[nP+1], block * KEY[nP+1], bool * r, int64_t length) {
		...
		for(int i = 1; i <= nP; ++i) for(int j = 1; j <= nP; ++j) if (i < j ) {
			if(i == party) {
				res.push_back(pool->enqueue([this, tKEY, tr, s, length, bucket_size, j]() {
					...
					for(int64_t k = 0; k < length*bucket_size; ++k) {
						// uint8_t data = garble(tKEY[j], tr, s[j], k, j);
						// io->send_data(j, &data, 1);
						data[k] = garble(tKEY[j], tr, s[j], k, j);
				...
				res.push_back(pool->enqueue([this, tMAC, tr, s, length, bucket_size, i]() {
					...
					for(int64_t k = 0; k < length*bucket_size; ++k) {
						...
						bool tmp = evaluate(data[k], tMAC[i], tr, k, i);
		...
		for(int i = 1; i <= nP; ++i) for(int j = 1; j<= nP; ++j) if( (i < j) and (i == party or j == party) ) {
			...
			res.push_back(pool->enqueue([this, tKEY, tKEYphi, phi, length, bucket_size, party2]() {
				block bH[2], tmpH[2];
				block *bH_send = new block[length*bucket_size];
				...
				io->send_data(party2, bH_send, length*bucket_size*sizeof(block));
				io->flush(party2);
				delete [] bH_send;
			}));
			res.push_back(pool->enqueue([this, tMAC, tMACphi, tr, length, bucket_size, party2]() {
				block bH;
				block *bH_receive = new block[length*bucket_size];
				io->recv_data(party2, bH_receive, length*bucket_size*sizeof(block));
				io->flush(party2);
				for(int k = 0; k < length*bucket_size; ++k) {
		...
		for(int64_t i = 0; i < length*bucket_size; ++i) xs[i] = tr[3*i];
		...
		int64_t *ind      = new int64_t[length*bucket_size];
		int64_t *location = new int64_t[length*bucket_size];
		bool    *d[nP+1];
		for(int i = 1; i <= nP; ++i)
			d[i] = new bool[length*(bucket_size-1)];
		for(int64_t i = 0; i < length*bucket_size; ++i)
			location[i] = i;
		PRG prg2(&S);
		prg2.random_data(ind, length*bucket_size*8);
		for(int64_t i = length*bucket_size-1; i>=0; --i) {
			int64_t index = ind[i] % (i+1);
			index = (index > 0) ? index : (-1 * index);
			int64_t tmp = location[i];
		...
		for(int64_t i = 0; i < length; ++i) {
			...
			for(int64_t j = 0; j < bucket_size-1; ++j)
			...
			for(int64_t k = 1; k < bucket_size; ++k) {
		...
		for(int i = 2; i <= nP; ++i)
			for(int64_t j = 0; j < (bucket_size-1)*length; ++j)
				d[1][j] = d[1][j]!=d[i][j];
