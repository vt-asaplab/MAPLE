...
class ORAM
{
    public:
        ...
        bool swap_block(int bucket_id, int position, int path_id, int path_position);
        void get_path(int path_id, int *full_path);
};
...
{
    this->depth_word        = ceil(log(file_block_size)/log(2));
    this->depth_file        = ceil(log(word_block_size<<3)/log(2));
    this->num_buckets_word  = file_block_size - 1;
    this->num_buckets_file  = (word_block_size<<3) - 1;
...
void ORAM::get_path(int path_id, int *full_path)
{
    int current_id = 0;
    full_path[0]   = 0;
    int count      = 0;
...
void ORAM::init_tree()
{
    ...
    for(int i = 0; i < this->num_buckets_word; ++i)
    {
        ...
        for(int j = 0; j < BUCKET_SIZE; ++j)
        {
            data[i][j] = new uint8_t[word_block_size<<1];
            memset(data[i][j], 0, word_block_size<<1);
        }
    }
}
...
bool ORAM::insert_block(int block_id, uint8_t *data, int path_id, int path_position)
{
    ...
    this->meta_data_word[current_id][offset].path_id  = assigned_path_id;
...
bool ORAM::swap_block(int bucket_id, int position, int path_id, int path_position)
{
    int current_id       = 0;
    int current_position = 0;
    int assigned_path_id = path_id;
    ...
    if(this->meta_data_file[current_id][offset].block_id != 0)
        return false;

    this->meta_data_file[current_id][offset].block_id  = this->meta_data_file[bucket_id][position].block_id;
    this->meta_data_file[current_id][offset].path_id   = assigned_path_id;
    this->meta_data_file[bucket_id][position].block_id = 0;
    this->meta_data_file[bucket_id][position].path_id  = 0;
    
    int full_position     = (bucket_id<<1) + position;
    int byte_old_position = full_position >> 3;
    int bit_old_position  = (full_position % 8);
    
    full_position         = (current_id<<1) + offset;
    int byte_new_position = full_position >> 3;
    int bit_new_position  = (full_position % 8);
    ...
    for(int i = 0; i < this->num_buckets_word; ++i)
    {
        for(int j = 0; j < BUCKET_SIZE; ++j)
        {
            bit = (data[i][j][byte_old_position] >> bit_old_position) & 0x01;
            data[i][j][byte_old_position] &= ~(1<<bit_old_position);
            data[i][j][byte_new_position] |= ((bit & 0x01) << bit_new_position);
        }
    }
